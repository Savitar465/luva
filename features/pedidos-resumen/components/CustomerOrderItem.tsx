...
export function CustomerOrderItem({ item }: Readonly<CustomerOrderItemProps>) {
  ...
  return (
    <li className="flex flex-col gap-4 rounded-2xl border border-[#d8eaec] bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between sm:p-6">
      <div className="flex items-center gap-4 sm:gap-6">
        <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-[#e6f6f7] text-[#241E20] sm:h-16 sm:w-16">
          {iconNode}
        </div>
        <div>
          <h2 className="text-xl font-bold text-[#241E20] sm:text-2xl">{item.nombre}</h2>
      ...
      <span className="text-2xl font-bold text-[#241E20] sm:text-3xl">Bs. {item.subtotal.toFixed(2)}</span>
    </li>
  );
}
